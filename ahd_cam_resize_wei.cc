...
const char *SSD_MODEL_PATH = "model/vgg16_eyeclose.rknn";
...
#define NUM_NPU_FD 2
    ...
    // fwrite(image_addr, 1, image_size_h, fout);
    // fclose(fout);
...
rknn_input_output_num print_model_info(rknn_context ctx)
{
    ...
    if (ret != RKNN_SUCC)
    {
        throw std::runtime_error("rknn_query fail! ret1");
    }
    ...
    for (int i = 0; i < io_num.n_input; i++)
    {
        ...
        if (ret != RKNN_SUCC)
        {
            throw std::runtime_error("rknn_query fail! ret2");
        }
    }
    ...
    for (int i = 0; i < io_num.n_output; i++)
    {
        ...
        if (ret != RKNN_SUCC)
        {
            throw std::runtime_error("rknn_query fail! ret3");
        }
    }
    return io_num;
}
...
static void *detect_thread_handler(void *arg)
{
    ...
    while (1)
    {
        if (image_id > image_id_prev)
        {
            ...
            // cout << typeid(*image_drawing_addr).name() << endl;
            ...
            try
            {
                ...
                for (unsigned long j = 0; j < dets.size(); ++j)
                {
                    ...
                    else
                    {
                        ...
                        int res = predict_one_pic(left_eye, rknn_ctx, rknn_io_num);
                        if (res == 1)
                        {
                    ...
                    {
                        // rknn_input_output_num io_num = print_model_info(ctx);
                        int res = predict_one_pic(right_eye, rknn_ctx, rknn_io_num);
                        if (res == 1)
                        {
