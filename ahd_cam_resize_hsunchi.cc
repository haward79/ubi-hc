...
//const char* SSD_MODEL_PATH = "model/ubi_yolov5s.rknn";
...
int main()
{
    ...
    // printf("image_size_h:%d",image_size_h);
    // fwrite(image_addr, 1, RGA_BUFFER_SCALED_SIZE, fout);
    // fclose(fout);
...
void rga_chn12_frame_cb(MEDIA_BUFFER mb)
{
    ...
    image_size_h = raw_stream_data_size;
    ...
    memcpy(image_addr, raw_stream_data_addr, image_size_h);
...
static void *detect_thread_handler(void *arg)
{
    while(1)
    {
        if(image_id > image_id_prev)
        {
            ...
            memcpy(image_drawing_addr, image_addr, image_size_h);
            crop_hFlip_image(image_drawing_addr, image_drawing_cropped_addr);

            FILE* fout = fopen("image.raw", "wb");
            printf("image_size_h:%d\n",image_size_h);
            fwrite(image_drawing_addr, 1, image_size_h, fout);
            fclose(fout);
            printf("[!!!] Saved a raw image to disk.\n");
            ...
            //raw->mat
            cv::Mat cv_img;
            cv_img.create(height,width,CV_32F);
            memcpy(cv_img.data,image_drawing_addr,height*width*3*sizeof(float));
            //mat->array2d
            // dlib::array2d<dlib::rgb_pixel> dlibImage;
            // dlib::assign_image(dlibImage, dlib::cv_image<rgb_pixel>(cv_img));
            // IplImage ipl_img = cvIplImage(cv_img);
            // cv_image<rgb_pixel> cimg(&ipl_img);

        
            // printf("processing image " );
            // load_image(img, "image.raw");

            // std::vector<rectangle> dets = detector(cimg);
            // printf("Number of faces detected: %d",dets.size()) ;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for(int i = 0; i < detect_result_group.count; i++)
                        {
                            ...
                            if(strcmp("car", det_result->name) == 0 && det_result->box.left >= MODEL_INPUT_SIZE / 2)
                            {
                                has_car = 1;
                                ...
                                draw_border((char *)image_drawing_cropped_addr, MODEL_INPUT_SIZE, MODEL_INPUT_SIZE, x, y, w, h);
                        ...
                        char min_pixel_distance_str[100];
                        sprintf(min_pixel_distance_str, "%d", min_pixel_distance);

                        // Put distance value on image.
                        put_text((char *)image_drawing_cropped_addr, MODEL_INPUT_SIZE, MODEL_INPUT_SIZE, 15, 15, min_pixel_distance_str);

                        // Write annotated image to disk.
                        FILE* fout = nullptr;
                        char filename[100];
                        sprintf(filename, "/root/output/%d.raw", image_id_prev);
                        printf("image_size_h:%d",RGA_BUFFER_SCALED_SIZE);
...
void draw_border(char* image_addr, int image_width, int image_height, int x, int y, int w, int h)
{
    int R = 255, G = 0, B = 0;
    ...
    for(int i=x; i<x+w; ++i)
    {
        ...
        image_addr[j*image_width*3 + i*3 + 0] = B;
        image_addr[j*image_width*3 + i*3 + 1] = G;
        image_addr[j*image_width*3 + i*3 + 2] = R;
    }
