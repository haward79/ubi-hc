...
const int CAMERA_ID_BS_LEFT = 1;
const int CAMERA_ID_BS_RIGHT = 2;
const int CAMERA_ID_ECD = 3;
...
JMPP_RAW_STREAM_S raw_stream_od, raw_stream_bsl, raw_stream_bsr, raw_stream_ecd;

int raw_stream_vi_chn_od, raw_stream_vi_chn_bsl, raw_stream_vi_chn_bsr, raw_stream_vi_chn_ecd;
int raw_stream_vi_pipe_od, raw_stream_vi_pipe_bsl, raw_stream_vi_pipe_bsr, raw_stream_vi_pipe_ecd;
...
// BSL.
uint32_t     bsl_raw_stream_data_size        = 0;
uint8_t*     bsl_raw_stream_data_addr        = 0;
uint64_t     bsl_raw_stream_data_timestamp   = 0;
uint32_t     bsl_raw_stream_data_width       = 0;
uint32_t     bsl_raw_stream_data_height      = 0;
IMAGE_TYPE_E bsl_raw_stream_data_color_space = IMAGE_TYPE_UNKNOW;

uint32_t     bsl_rga_image_size              = 0;
uint64_t     bsl_rga_image_timestamp         = 0;
uint32_t     bsl_rga_image_width             = 0;
uint32_t     bsl_rga_image_height            = 0;
IMAGE_TYPE_E bsl_rga_image_color_space       = IMAGE_TYPE_UNKNOW;

// These memory will be released at the end of main function.
uint8_t*     bsl_rga_image_buffer_addr       = (uint8_t*)malloc(OD_RGA_BUFFER_OUTPUT_SIZE);
uint8_t*     bsl_image_temp_addr             = (uint8_t*)malloc(OD_RGA_BUFFER_OUTPUT_SIZE);
uint8_t*     bsl_image_addr                  = (uint8_t*)malloc(OD_RGA_BUFFER_CROPPED_SIZE);

int          bsl_rga_image_id = -1;
int          bsl_rga_image_prev_id = -1;


// BSR.
uint32_t     bsr_raw_stream_data_size        = 0;
uint8_t*     bsr_raw_stream_data_addr        = 0;
uint64_t     bsr_raw_stream_data_timestamp   = 0;
uint32_t     bsr_raw_stream_data_width       = 0;
uint32_t     bsr_raw_stream_data_height      = 0;
IMAGE_TYPE_E bsr_raw_stream_data_color_space = IMAGE_TYPE_UNKNOW;

uint32_t     bsr_rga_image_size              = 0;
uint64_t     bsr_rga_image_timestamp         = 0;
uint32_t     bsr_rga_image_width             = 0;
uint32_t     bsr_rga_image_height            = 0;
IMAGE_TYPE_E bsr_rga_image_color_space       = IMAGE_TYPE_UNKNOW;

// These memory will be released at the end of main function.
uint8_t*     bsr_rga_image_buffer_addr       = (uint8_t*)malloc(OD_RGA_BUFFER_OUTPUT_SIZE);
uint8_t*     bsr_image_temp_addr             = (uint8_t*)malloc(OD_RGA_BUFFER_OUTPUT_SIZE);
uint8_t*     bsr_image_addr                  = (uint8_t*)malloc(OD_RGA_BUFFER_CROPPED_SIZE);

int          bsr_rga_image_id = -1;
int          bsr_rga_image_prev_id = -1;
...
rknn_context          rknn_ctx_od, rknn_ctx_bsl, rknn_ctx_bsr, rknn_ctx_ecd;
rknn_input_output_num rknn_io_num_od, rknn_io_num_bsl, rknn_io_num_bsr, rknn_io_num_ecd;
...
void rga_chn13_frame_cb(MEDIA_BUFFER mb);
void rga_chn14_frame_cb(MEDIA_BUFFER mb);
void rga_chn15_frame_cb(MEDIA_BUFFER mb);
...
int main()
{
    ...
    int r = 0;
    int model_od_len = 0, model_bsl_len = 0, model_bsr_len = 0, model_ecd_len = 0;
    unsigned char *model_od = nullptr;
    unsigned char *model_bsl = nullptr;
    unsigned char *model_bsr = nullptr;
    ...
    // Enable BS Left.
    {
        printf(ANSI_COLOR_GREEN "BS is ENABLED in configuration file.\n" ANSI_COLOR_RESET);

        // Open raw stream.
        // Raw stream is in NV16 color space.
        // User has to call Rockchip native API to get frame data.
        r = jmpp_raw_stream_open(CAMERA_ID_BS_LEFT, &raw_stream_bsl);

        if(r != 0)
        {
            printf("[BSL] Can NOT open RAW stream.\n");
        }
        ...
        // There are different ways to get frame data, either using RK_MPI_SYS_GetMediaBuffer()
        // or RK_MPI_SYS_RegisterOutCb(). But application may have to create thread when calling them.
        raw_stream_vi_pipe_bsl = jmpp_raw_stream_vi_pipe(CAMERA_ID_BS_LEFT, &raw_stream_bsl);
        raw_stream_vi_chn_bsl = jmpp_raw_stream_vi_chn(CAMERA_ID_BS_LEFT, &raw_stream_bsl);
        ...
        // RGA channel 12 .. 15 can be used with rockchip native API.
        r = RK_MPI_RGA_CreateChn(13, &stRgaAttr);
        
        if(r)
        {
            printf("[BSL] ERROR: Create rga[13] falied! ret=%d\n", r);
        }

        // Register a callback when there is a data on rga channel 13.
        MPP_CHN_S stEncChn;

        stEncChn.enModId = RK_ID_RGA;
        stEncChn.s32ChnId = 13;
        
        r = RK_MPI_SYS_RegisterOutCb(&stEncChn, rga_chn13_frame_cb);

        if(r)
        {
            printf("[BSL] ERROR: Register cb for RGA CB error! code:%d\n", r);
        }
        ...
        MPP_CHN_S stSrcChn;
        stSrcChn.enModId = RK_ID_VI;
        stSrcChn.s32DevId = raw_stream_vi_pipe_bsl;
        stSrcChn.s32ChnId = raw_stream_vi_chn_bsl;
        MPP_CHN_S stDestChn;
        stDestChn.enModId = RK_ID_RGA;
        stDestChn.s32DevId = 0;
        stDestChn.s32ChnId = 13;
        
        r = RK_MPI_SYS_Bind(&stSrcChn, &stDestChn);
        
        if(r)
        {
            printf("[BSL] ERROR: Bind vi[%d:%d] and rga[13] failed! ret=%d\n", raw_stream_vi_pipe_bsl, raw_stream_vi_chn_bsl, r);
        }

        // Preload BSL rknn model.
        /* printf("Loading BSL model ...\n");
        model_bsl = load_model(OD_MODEL_PATH, &model_bsl_len);
        r = rknn_init(&rknn_ctx_bsl, model_bsl, model_bsl_len, 0);
        ...
        {
            // Get Model Input Output Info
            r = rknn_query(rknn_ctx_bsl, RKNN_QUERY_IN_OUT_NUM, &rknn_io_num_bsl, sizeof(rknn_io_num_bsl));
            ...
            {
                printf("Model loaded successfully.\n");
                printf("Model input num: %d, output num: %d\n", rknn_io_num_bsl.n_input, rknn_io_num_bsl.n_output);

                {
                    printf("Model input tensors:\n");
                    
                    rknn_tensor_attr input_attrs[rknn_io_num_bsl.n_input];
                    memset(input_attrs, 0, sizeof(input_attrs));
                    
                    for(unsigned int i = 0; i < rknn_io_num_bsl.n_input; i++)
                    {
                        input_attrs[i].index = i;
                        r = rknn_query(rknn_ctx_bsl, RKNN_QUERY_INPUT_ATTR, &(input_attrs[i]), sizeof(rknn_tensor_attr));
...

                {
                    ...
                    rknn_tensor_attr output_attrs[rknn_io_num_bsl.n_output];
                    memset(output_attrs, 0, sizeof(output_attrs));
                    
                    for(unsigned int i = 0; i < rknn_io_num_bsl.n_output; i++)
                    {
                        output_attrs[i].index = i;
                        r = rknn_query(rknn_ctx_bsl, RKNN_QUERY_OUTPUT_ATTR, &(output_attrs[i]), sizeof(rknn_tensor_attr));
    ...
    // Enable BS Right.
    {
        ...
        // Raw stream is in NV16 color space.
        // User has to call Rockchip native API to get frame data.
        r = jmpp_raw_stream_open(CAMERA_ID_BS_RIGHT, &raw_stream_bsr);

        if(r != 0)
        {
            printf("[BSR] Can NOT open RAW stream.\n");
        }
        ...
        // There are different ways to get frame data, either using RK_MPI_SYS_GetMediaBuffer()
        // or RK_MPI_SYS_RegisterOutCb(). But application may have to create thread when calling them.
        raw_stream_vi_pipe_bsr = jmpp_raw_stream_vi_pipe(CAMERA_ID_BS_RIGHT, &raw_stream_bsr);
        raw_stream_vi_chn_bsr = jmpp_raw_stream_vi_chn(CAMERA_ID_BS_RIGHT, &raw_stream_bsr);
        ...
        if(r)
        {
            printf("[BSR] ERROR: Create rga[14] falied! ret=%d\n", r);
        }
        ...
        if(r)
        {
            printf("[BSR] ERROR: Register cb for RGA CB error! code:%d\n", r);
        }

        // starting the VI -> RGA[12], VI -> RGA[14] pipeline data.
        // VI automatically starts capturing data after binding is completed.
        MPP_CHN_S stSrcChn;
        stSrcChn.enModId = RK_ID_VI;
        stSrcChn.s32DevId = raw_stream_vi_pipe_bsr;
        stSrcChn.s32ChnId = raw_stream_vi_chn_bsr;
        MPP_CHN_S stDestChn;
        stDestChn.enModId = RK_ID_RGA;
        stDestChn.s32DevId = 0;
        ...
        if(r)
        {
            printf("[BSR] ERROR: Bind vi[%d:%d] and rga[14] failed! ret=%d\n", raw_stream_vi_pipe_bsr, raw_stream_vi_chn_bsr, r);
        }

        // Preload BSR rknn model.
        /* printf("Loading BSR model ...\n");
        model_bsr = load_model(OD_MODEL_PATH, &model_bsr_len);
        r = rknn_init(&rknn_ctx_bsr, model_bsr, model_bsr_len, 0);
        ...
        {
            // Get Model Input Output Info
            r = rknn_query(rknn_ctx_bsr, RKNN_QUERY_IN_OUT_NUM, &rknn_io_num_bsr, sizeof(rknn_io_num_bsr));
            ...
            {
                printf("Model loaded successfully.\n");
                printf("Model input num: %d, output num: %d\n", rknn_io_num_bsr.n_input, rknn_io_num_bsr.n_output);

                {
                    printf("Model input tensors:\n");
                    
                    rknn_tensor_attr input_attrs[rknn_io_num_bsr.n_input];
                    memset(input_attrs, 0, sizeof(input_attrs));
                    
                    for(unsigned int i = 0; i < rknn_io_num_bsr.n_input; i++)
                    {
                        input_attrs[i].index = i;
                        r = rknn_query(rknn_ctx_bsr, RKNN_QUERY_INPUT_ATTR, &(input_attrs[i]), sizeof(rknn_tensor_attr));
...

                {
                    ...
                    rknn_tensor_attr output_attrs[rknn_io_num_bsr.n_output];
                    memset(output_attrs, 0, sizeof(output_attrs));
                    
                    for(unsigned int i = 0; i < rknn_io_num_bsr.n_output; i++)
                    {
                        output_attrs[i].index = i;
                        r = rknn_query(rknn_ctx_bsr, RKNN_QUERY_OUTPUT_ATTR, &(output_attrs[i]), sizeof(rknn_tensor_attr));
    ...
    if(get_config_ECD_enable())
    {
        ...
        // RGA[15] color space conversation.
        RGA_ATTR_S stRgaAttr;
        stRgaAttr.bEnBufPool = RK_TRUE;
        stRgaAttr.u16BufPoolCnt = 2;  // Please do not set too many. 2 is default.
        stRgaAttr.u16Rotaion = 0;
        ...
        // RGA channel 12 .. 15 can be used with rockchip native API.
        r = RK_MPI_RGA_CreateChn(15, &stRgaAttr);
        
        if(r)
        {
            printf("[ECD] ERROR: Create rga[15] falied! ret=%d\n", r);
        }

        // Register a callback when there is a data on rga channel 15.
        MPP_CHN_S stEncChn;

        stEncChn.enModId = RK_ID_RGA;
        stEncChn.s32ChnId = 15;
        
        r = RK_MPI_SYS_RegisterOutCb(&stEncChn, rga_chn15_frame_cb);
        ...
        stDestChn.enModId = RK_ID_RGA;
        stDestChn.s32DevId = 0;
        stDestChn.s32ChnId = 15;
        
        r = RK_MPI_SYS_Bind(&stSrcChn, &stDestChn);
        
        if(r)
        {
            printf("[ECD] ERROR: Bind vi[%d:%d] and rga[15] failed! ret=%d\n", raw_stream_vi_pipe_od, raw_stream_vi_chn_od, r);
        }
...
void rga_chn13_frame_cb(MEDIA_BUFFER mb)
{
    ...
    if(chn != 13)   // Depending on the setup.
    {
        printf("[BSL] RGA channel invalid\n");
    }
    ...
    if(r)
    {
        printf("[BSL] Warn: Get image info failed! r = %d\n", r);
    }
    
    bsl_raw_stream_data_addr = (uint8_t  *) RK_MPI_MB_GetPtr(mb);
    bsl_raw_stream_data_size = RK_MPI_MB_GetSize(mb);
    bsl_raw_stream_data_timestamp = RK_MPI_MB_GetTimestamp(mb);
    bsl_raw_stream_data_width = stImageInfo.u32Width;
    bsl_raw_stream_data_height = stImageInfo.u32Height;
    bsl_raw_stream_data_color_space = stImageInfo.enImgType;

    // Do image and image info copy.
    bsl_rga_image_size = bsl_raw_stream_data_size;
    bsl_rga_image_timestamp = bsl_raw_stream_data_timestamp;
    bsl_rga_image_width = bsl_raw_stream_data_width;
    bsl_rga_image_height = bsl_raw_stream_data_height;
    bsl_rga_image_color_space = bsl_raw_stream_data_color_space;
    memcpy(bsl_rga_image_buffer_addr, bsl_raw_stream_data_addr, bsl_rga_image_size);

    // MUST release memory as soon as possible!
    RK_MPI_MB_ReleaseBuffer(mb);

    ++bsl_rga_image_id;
    //printf("[BSL][%d] New image generated.\n", bsl_rga_image_id);
}
...
void rga_chn14_frame_cb(MEDIA_BUFFER mb)
{
    ...
    {
        printf("[BSR] RGA channel invalid\n");
    }
    ...
    if(r)
    {
        printf("[BSR] Warn: Get image info failed! r = %d\n", r);
    }
    
    bsr_raw_stream_data_addr = (uint8_t  *) RK_MPI_MB_GetPtr(mb);
    bsr_raw_stream_data_size = RK_MPI_MB_GetSize(mb);
    bsr_raw_stream_data_timestamp = RK_MPI_MB_GetTimestamp(mb);
    bsr_raw_stream_data_width = stImageInfo.u32Width;
    bsr_raw_stream_data_height = stImageInfo.u32Height;
    bsr_raw_stream_data_color_space = stImageInfo.enImgType;

    // Do image and image info copy.
    bsr_rga_image_size = bsr_raw_stream_data_size;
    bsr_rga_image_timestamp = bsr_raw_stream_data_timestamp;
    bsr_rga_image_width = bsr_raw_stream_data_width;
    bsr_rga_image_height = bsr_raw_stream_data_height;
    bsr_rga_image_color_space = bsr_raw_stream_data_color_space;
    memcpy(bsr_rga_image_buffer_addr, bsr_raw_stream_data_addr, bsr_rga_image_size);

    // MUST release memory as soon as possible!
    RK_MPI_MB_ReleaseBuffer(mb);

    ++bsr_rga_image_id;
    //printf("[BSR][%d] New image generated.\n", bsr_rga_image_id);
}
...
void rga_chn15_frame_cb(MEDIA_BUFFER mb)
{
    ...
    if(chn != 15)   // Depending on the setup.
    {
...
static void *detect_thread_handler(void *arg)
{
    ...
    while(1)
    {
        long loop_timer_start = getCurrentTimeMsec();
        ...
        // BSL.
        if(bsl_rga_image_id > bsl_rga_image_prev_id)
        {
            printf(ANSI_COLOR_GREEN "[BSL] Process an image.\n" ANSI_COLOR_RESET);

            bsl_rga_image_prev_id = bsl_rga_image_id;

            // Prevent from rga_chn13_frame_cb() overwrite.
            memcpy(bsl_image_temp_addr, bsl_rga_image_buffer_addr, bsl_rga_image_size);

            // Flip image horizontally.
            hFlip_image(bsl_image_temp_addr, OD_RGA_OUTPUT_WIDTH, OD_RGA_OUTPUT_HEIGHT);

            // Crop image from 16:9 to 1:1 .
            hCrop_image(bsl_image_temp_addr, bsl_image_addr, OD_RGA_OUTPUT_WIDTH, OD_MODEL_INPUT_WIDTH, OD_MODEL_INPUT_HEIGHT, OD_RGA_OUTPUT_CROP_OFFSET);
            ...
            // BSL.
            {
                ...
                input[0].size = OD_RGA_BUFFER_CROPPED_SIZE;
                input[0].fmt = RKNN_TENSOR_NHWC;
                input[0].buf = bsl_image_addr;

                r = rknn_inputs_set(rknn_ctx_od, rknn_io_num_od.n_input, input);
                
                if(r < 0)
                {
                    printf("[BSL] Failed to set rknn input! Returned %d.\n", r);
                }
                else
                {
                    // Run rknn.
                    ...
                    if(r < 0)
                    {
                        printf("[BSL] Failed to run rknn! Returned %d.\n", r);
                    }
                    else
                    {
                        // Get Output.
                        ...
                        if(r < 0)
                        {
                            printf("[BSL] Failed to get rknn output! Returned %d.\n", r);
                        }
                        else
                        {
                            // Deal with output.
                            ...
                            for(int i = 0; i < detect_result_group.count; i++)
                            {
                                ...
                                if(strcmp("car", det_result->name) == 0)
                                {
                                    printf("[BSL]  %s @ (top=%d, right=%d, bottom=%d, left=%d) confidence=%f\n",
                                        det_result->name,
                                        det_result->box.top,
                                        det_result->box.right,
                                        det_result->box.bottom,
                                    ...
                                    // Draw annotation on image.
                                    //if(get_config_BSL_objectMark_enable())
                                    {
                                        int x = det_result->box.left;
                                        int y = det_result->box.top;
                                        int w = det_result->box.right - det_result->box.left;
                                        ...
                                        draw_border((char *)bsl_image_addr, OD_MODEL_INPUT_WIDTH, OD_MODEL_INPUT_HEIGHT, x, y, w, h, COLOR_RED);
                                    }
                                }
                            }

                            // Write annotated image to disk.
                            //if(get_config_BSL_recording_enable())
                            {
                                FILE* fout = nullptr;
                                char filename[100] = "";
                                char output_path[100] = "";

                                strcat(output_path, "/root/output" /*get_config_BSL_recording_path()*/);
                            ...
                            long timer_interval = getCurrentTimeMsec() - timer_start;
                            printf("[BSL][%d] process takes %ld ms.\n", bsl_rga_image_prev_id, timer_interval);
                        }
                    }
                }
            }
        ...
        // BSR.
        if(bsr_rga_image_id > bsr_rga_image_prev_id)
        {
            printf(ANSI_COLOR_GREEN "[BSR] Process an image.\n" ANSI_COLOR_RESET);

            bsr_rga_image_prev_id = bsr_rga_image_id;

            // Prevent from rga_chn14_frame_cb() overwrite.
            memcpy(bsr_image_temp_addr, bsr_rga_image_buffer_addr, bsr_rga_image_size);

            // Flip image horizontally.
            hFlip_image(bsr_image_temp_addr, OD_RGA_OUTPUT_WIDTH, OD_RGA_OUTPUT_HEIGHT);

            // Crop image from 16:9 to 1:1 .
            hCrop_image(bsr_image_temp_addr, bsr_image_addr, OD_RGA_OUTPUT_WIDTH, OD_MODEL_INPUT_WIDTH, OD_MODEL_INPUT_HEIGHT, OD_RGA_OUTPUT_CROP_OFFSET);
            ...
            // BSR.
            {
                ...
                input[0].size = OD_RGA_BUFFER_CROPPED_SIZE;
                input[0].fmt = RKNN_TENSOR_NHWC;
                input[0].buf = bsr_image_addr;

                r = rknn_inputs_set(rknn_ctx_od, rknn_io_num_od.n_input, input);
                
                if(r < 0)
                {
                    printf("[BSR] Failed to set rknn input! Returned %d.\n", r);
                }
                else
                {
                    // Run rknn.
                    ...
                    if(r < 0)
                    {
                        printf("[BSR] Failed to run rknn! Returned %d.\n", r);
                    }
                    else
                    {
                        // Get Output.
                        ...
                        if(r < 0)
                        {
                            printf("[BSR] Failed to get rknn output! Returned %d.\n", r);
                        }
                        else
                        {
                            // Deal with output.
                            ...
                            for(int i = 0; i < detect_result_group.count; i++)
                            {
                                ...
                                if(strcmp("car", det_result->name) == 0)
                                {
                                    printf("[BSR]  %s @ (top=%d, right=%d, bottom=%d, left=%d) confidence=%f\n",
                                        det_result->name,
                                        det_result->box.top,
                                        det_result->box.right,
                                        det_result->box.bottom,
                                    ...
                                    // Draw annotation on image.
                                    //if(get_config_BSR_objectMark_enable())
                                    {
                                        int x = det_result->box.left;
                                        int y = det_result->box.top;
                                        int w = det_result->box.right - det_result->box.left;
                                        ...
                                        draw_border((char *)bsr_image_addr, OD_MODEL_INPUT_WIDTH, OD_MODEL_INPUT_HEIGHT, x, y, w, h, COLOR_RED);
                                    }
                                }
                            }

                            // Write annotated image to disk.
                            //if(get_config_BSR_recording_enable())
                            {
                                FILE* fout = nullptr;
                                char filename[100] = "";
                                char output_path[100] = "";

                                strcat(output_path, "/root/output" /*get_config_BSR_recording_path()*/);
                            ...
                            long timer_interval = getCurrentTimeMsec() - timer_start;
                            printf("[BSR][%d] process takes %ld ms.\n", bsr_rga_image_prev_id, timer_interval);
                        }
                    }
                }
            }
        ...
        long loop_timer_interval = getCurrentTimeMsec() - loop_timer_start;
        printf("[DT] one loop process takes %ld ms.\n", loop_timer_interval);
